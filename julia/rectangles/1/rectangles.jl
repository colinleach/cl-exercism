...
function rectangles(strings)
    ...
    connected_sides(pt1, pt2) = all([Pt(pt1.c, row) ∈ vert for row in pt1.r:pt2.r]) &&
                        all([Pt(pt2.c, row) ∈ vert for row in pt1.r:pt2.r])

    connected_top_bottom(pt1, pt2) = all([Pt(col, pt1.r) ∈ horiz for col in pt1.c:pt2.c]) &&
                            all([Pt(col, pt2.r) ∈ horiz for col in pt1.c:pt2.c])

    [(pt1, pt2) for (pt1, pt2) in possibles 
                if connected_sides(pt1, pt2) && connected_top_bottom(pt1, pt2)] |> length
end
