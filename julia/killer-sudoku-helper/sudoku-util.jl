...
function combinations_in_cage(total, count, exclude=[])
    digits = setdiff(collect(1:9), exclude)
    digits = digits[digits .<= total]
    sort([x for x in combinations(digits, count) if sum(x)==total])
end